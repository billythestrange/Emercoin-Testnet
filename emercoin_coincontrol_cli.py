...
class CoinControlCLI:
    ...
    def prompt_amount(self, prompt_text: str, max_amount: Optional[Decimal] = None) -> Optional[Decimal]:
        ...
        while True:
            ...
            try:
                ...
                if max_amount and amount > max_amount:
                    self.print_error(f"Betrag darf maximal {self.format_amount(max_amount)} sein!")
                    continue
    ...
    def connect_to_wallet(self) -> bool:
        ...
        self.print_success(f"Verbunden! Guthaben: {self.format_amount(self.wallet_balance)}")
    ...
    def _select_by_amount(self, utxos: List[Dict], total_available: Decimal) -> List[Dict]:
        """Benutzer wählt UTXOs nach Gesamtbetrag"""
        target = self.prompt_amount(
            "Benötigter Betrag (Coins werden nach Größe sortiert)",
            total_available
        )
    ...
    def create_transaction(
        ...
        if change < 0:
            self.print_error(f"Nicht genug Guthaben! Fehlbetrag: {self.format_amount(abs(change))}")
            return None
        
        # Outputs vorbereiten
        outputs = {
            recipient_address: float(send_amount)
        }
        ...
        if change > Decimal("0.00000001"):  # Nur wenn Dust-Schwelle überschritten
            ...
            print(f"   Change: {self.format_amount(change)} (unter Dust-Schwelle, wird vernichtet)")
        ...
        self.print_success(f"Raw-Transaktion erstellt!")
    ...
    def sign_and_send_transaction(self, tx_hex: str) -> Optional[str]:
        ...
        self.print_success(f"Transaktion gesendet!")
        print(f"\n🎉 TXID: {txid}")
    ...
    def run(self):
        ...
        send_amt = self.prompt_amount(
            f"Sendebetrag (max. {self.format_amount(max_send)})",
            max_send
        )
        if send_amt is None:
            sys.exit(1)
